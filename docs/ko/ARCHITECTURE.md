...
>
> 이 문서는 패널 자체 — 10개 에이전트와 그걸 돌리는 파이프라인 — 를 다룬다. 패널에 애초에 무엇을 먹일지(자연어 컨텍스트, 검증된 리서치, 라운드를 넘나드는 사람이 선택한 메모리)는 [`CONTEXT-MEMORY-ARCHITECTURE.md`](CONTEXT-MEMORY-ARCHITECTURE.md)에서 따로 다룬다 — 목표 아키텍처이며 아직 구현되지 않았다.
