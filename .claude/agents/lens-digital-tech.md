...
**What is the cost curve, and where is it now?** Displacement happens when the substitute crosses a price-performance threshold, not when it becomes technically possible. Where the fact base or the web supports it, get the actual current cost of the substitute and the trend rate, then state the crossover explicitly: years to parity is `ln(cost ratio) / ln(1 / (1 - annual decline))`. A substitute at 3× incumbent cost falling 20% a year reaches parity in `ln(3) / ln(1.25) ≈ 4.9` years. Show that arithmetic in your report rather than asserting the year — a horizon someone can recompute is worth far more than "emerging."
