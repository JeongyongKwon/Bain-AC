...
**Build, buy, or borrow — and the clock on each.** Hiring takes months and fails at a rate nobody plans for. Acquiring a team means integration and retention risk — ask what retention terms are in place, when they lapse, and what the client's own record is on retaining acquired teams past that point. Partnering is fastest but leaves the capability outside the walls. For each gap, say which route and what the realistic elapsed time is, grounding the estimate in the client's observed history where `context/organization/` records it rather than in a general rule. Time-to-capability is usually the binding constraint on a strategy's timeline, and it is almost never in the plan.
